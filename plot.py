...
from pymatgen.io.vaspio.vasp_output import Vasprun, Procar
...
if __name__ == "__main__":
    ...
    for iat in data:
        for k in data[iat]:
            if not sdata.has_key(k):
                sdata[k] = dict()
            for b in data[iat][k]["bands"]:
                if not sdata[k].has_key(b):
                    sdata[k][b] = dict()
                for ao, val in data[iat][k]["bands"][b].items():
                    if sdata[k][b].has_key(ao):
                        sdata[k][b][ao] += val
                    else:
                        sdata[k][b][ao] = val
    ...
    for b in range(bands.nb_bands):
        for k in range(len(bands.kpoints)):
            sc = sdata[k + 1][b + 1]["s"]**2
            pxpyc = sdata[k + 1][b + 1]["px"]**2 + sdata[k + 1][b + 1]["py"]**2
            pzc = sdata[k + 1][b + 1]["pz"]**2
            tot = sc + pxpyc + pzc
            if tot != 0.0:
